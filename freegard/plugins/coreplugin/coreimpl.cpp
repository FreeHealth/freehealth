...
CoreImpl::CoreImpl(QObject *parent) :
        ...
        m_PadTools(0)
{
    setObjectName("FreeGuard::Core");
    m_Settings = new SettingsPrivate(this);
    m_Settings->setPath(ISettings::UpdateUrl, Utils::Constants::FREEGUARD_UPDATE_URL);
    m_Settings->setPath(ISettings::SplashScreen, Constants::FREEGUARD_SPLASHSCREEN);

    m_Theme = new ThemePrivate(this);
    m_Theme->setThemeRootPath(m_Settings->path(ISettings::ThemeRootPath));

    m_Theme->createSplashScreen(Constants::FREEGUARD_SPLASHSCREEN);
