...
const char* const  FDA_WORKINGPATH             = "/../global_resources/tmp/FdaRawSources/";
const char* const  FDA_DRUGS_DATABASE_FILENAME =  "/../global_resources/databases/drugs/drugs-en_US.db";
const char* const  FDA_DRUGS_DATABASE_NAME     =  "FDA_US";
const char* const  SEPARATOR                   = "|||";

const char* const  FILE_FDA_DRUGS_DATABASE_SCHEME     = "/../global_resources/sql/usa_db_creator.sql";
const char* const  FILE_FDA_DRUGS_DATABASE_SCHEME2    = "/../global_resources/sql/usa_db_finalize.sql";
    ...
    ui(new Ui::FdaDrugsDatabaseWidget)
{
    ...
    ui->setupUi(this);
    m_WorkingPath = QDir::cleanPath(qApp->applicationDirPath() + Constants::MACBUNDLE + FDA_WORKINGPATH) + QDir::separator();
...
bool FdaDrugsDatabaseWidget::downloadFiles()
{
    ...
    drug(const QString &line)
    {
        ...
        if (globalStrength.contains(";")) {
            ...
            if (globalStrength.contains("(")) {
                ...
                QRegExp reg("\(*;*\)", Qt::CaseSensitive, QRegExp::Wildcard);
                t.remove(reg);
                strengths = t.split(";");
            } else {
...
{
    if (!Tools::connectDatabase(FDA_DRUGS_DATABASE_NAME, FDA_DRUGS_DATABASE_FILENAME))
        return false;

    // create database structure
    if (!Tools::executeSqlFile(FDA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + Constants::FILE_DRUGS_DATABASE_SCHEME)) {
...
bool FdaDrugsDatabaseWidget::populateDatabase()
{
    ...
    if (!Tools::executeSqlFile(FDA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + FILE_FDA_DRUGS_DATABASE_SCHEME)) {
        Utils::Log::addError(this, "Can create FDA DB.", __FILE__, __LINE__);
        return false;
    }
    ...
    foreach(const QString &f, files) {
        if (!Tools::executeProcess(QString("sqlite3 -separator \"%1\" \"%2\" \".import \"%3\" %4\"")
            .arg(SEPARATOR, qApp->applicationDirPath() + Constants::MACBUNDLE + FDA_DRUGS_DATABASE_FILENAME, m_WorkingPath + f , QFileInfo(f).baseName().remove("FDA_")))) {
            return false;
        }
    }
    ...
    progressDialog.show();
    if (!Tools::executeSqlFile(FDA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + FILE_FDA_DRUGS_DATABASE_SCHEME2, &progressDialog)) {
        Utils::Log::addError(this, "Can create FDA DB.", __FILE__, __LINE__);
        return false;
    }
...
bool FdaDrugsDatabaseWidget::linkMolecules()
{
    ...
    Utils::Log::addMessage(this, "Getting Drugs Compostion 31418 distinct molecules");
    QString req = "SELECT DISTINCT `MOLECULE_CODE`, `MOLECULE_NAME` FROM `COMPOSITION`;";
    QSqlQuery mols(req, us);
    if (mols.exec(req)) {
        while (mols.next()) {
    ...
    // Associate Mol <-> ATC for drugs with one molecule only
    QStringList unfound;
    QMultiHash<int, int> mol_atc = Tools::moleculeLinker(FDA_DRUGS_DATABASE_NAME, "en", &unfound, mols_id, corrected);
    qWarning() << "unfound" << unfound.count();

    // Save to links to drugs database
    us.transaction();
    Tools::executeSqlQuery("DELETE FROM LK_MOL_ATC;", FDA_DRUGS_DATABASE_NAME);
    foreach(int mol, mol_atc.uniqueKeys()) {
        foreach(int atc, mol_atc.values(mol)) {
            ...
            Tools::executeSqlQuery(req, FDA_DRUGS_DATABASE_NAME);
        }
    }
    us.commit();
