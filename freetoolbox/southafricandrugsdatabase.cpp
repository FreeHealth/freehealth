...
const char* const  ZA_WORKINGPATH       = "/../global_resources/tmp/ZARawSources/";

const char* const  ZA_DRUGS_DATABASE_NAME     = "SAEPI_ZA";
const char* const  ZA_DRUGS_DATABASE_FILENAME =  "/../global_resources/databases/drugs/drugs-en_ZA.db";
const char* const  ZA_UIDS_FILE               = "/../global_resources/sql/south_african_uids.csv";

const char* const  FILE_ZA_DRUGS_DB_SCRIPT1   = "/../global_resources/sql/za_db_preparation.sql";
const char* const  FILE_ZA_DRUGS_DB_SCRIPT2   = "/../global_resources/sql/za_db_finalize.sql";
        ...
        QWidget(parent)
{
    ...
    ui->setupUi(this);
    m_WorkingPath = QDir::cleanPath(qApp->applicationDirPath() + Constants::MACBUNDLE + ZA_WORKINGPATH) + QDir::separator();
...
bool SouthAfricanDrugsDatabase::downloadFiles()
{
...
{
    if (!Tools::connectDatabase(ZA_DRUGS_DATABASE_NAME, ZA_DRUGS_DATABASE_FILENAME))
        return false;

    // create database structure
    if (!Tools::executeSqlFile(ZA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + Constants::FILE_DRUGS_DATABASE_SCHEME)) {
...
bool SouthAfricanDrugsDatabase::populateDatabase()
{
    ...
    {
        QFile uids(qApp->applicationDirPath() + Constants::MACBUNDLE + ZA_UIDS_FILE);
...
//        }
    ...
    foreach(Drug *dr, drugs) {
        // Insert the drug
        QString tmp = dr->name;
        req = QString("INSERT INTO `DRUGS` (`UID`, `NAME`, `MARKETED`, `LINK_SPC`) VALUES (%1, '%2', %3, '%4')")
              .arg(drugs_uids.value(dr->name))
              .arg(tmp.replace("'", "''"))
              .arg(1)
              .arg("http://home.intekom.com" + m_Drug_Link.value(dr->name))
              ;
        Tools::executeSqlQuery(req, ZA_DRUGS_DATABASE_NAME);
        foreach(const QString &mol, dr->inns) {
            req = QString("INSERT INTO `COMPOSITION` (`UID`, `MOLECULE_NAME`, `MOLECULE_CODE`) VALUES (%1, '%2', %3)")
                  .arg(drugs_uids.value(dr->name))
                  .arg(mol)
...
//    Tools::executeSqlFile(ZA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + FILE_ZA_DRUGS_DB_SCRIPT1);
    Tools::executeSqlFile(ZA_DRUGS_DATABASE_NAME, Constants::MACBUNDLE + FILE_ZA_DRUGS_DB_SCRIPT2);
...
bool SouthAfricanDrugsDatabase::linkMolecules()
{
    ...
    // 28 July 2010
    // Using the new dcTools::englishMoleculeLinker()
    // 1198 distinct mols
    // Hand association: 27
    // Found: 568, Left: 631
    ...
    QString req = "SELECT DISTINCT `MOLECULE_CODE`, `MOLECULE_NAME` FROM `COMPOSITION`";
    QSqlQuery mols(za);
    if (mols.exec(req)) {
        while (mols.next()) {
    ...
    QStringList unfound;
    QMultiHash<int, int> mol_atc = Tools::moleculeLinker(ZA_DRUGS_DATABASE_NAME, "en", &unfound, mols_id, corrected);
    qWarning() << "unfound" << unfound.count();

    // Save to links to drugs database
    za.transaction();
    Tools::executeSqlQuery("DELETE FROM LK_MOL_ATC;", ZA_DRUGS_DATABASE_NAME);
