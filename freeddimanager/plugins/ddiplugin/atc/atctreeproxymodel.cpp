...
AtcTreeModel *AtcTreeModel::_instance = 0;
namespace {
...
AtcTreeModel::AtcTreeModel(QObject * parent) :
    ...
    d(new AtcTreeModelPrivate(this))
{
    _instance = this;
}
...
bool AtcTreeModel::initialize()
{
    d->getTree();
    return true;
}

QVariant AtcTreeModel::data(const QModelIndex &index, int role) const
{
    if (!index.isValid())
        return QVariant();
...
QStringList AtcTreeModel::getAtcLabel(const QStringList &codes)
{
    QStringList list;
...
bool AtcTreeModel::insertAtcCodeToDatabase(const QString &connectionName)
{
    // Start transaction
