...
static inline QString workingPath()     {return QString();}//QDir::cleanPath(settings()->value(Core::Constants::S_TMP_PATH).toString() + "/ZARawSources/") + QDir::separator();}
static inline QString atcCsvFile()      {return QString();}//QDir::cleanPath(settings()->value(Core::Constants::S_GITFILES_PATH).toString() + QString(Core::Constants::ATC_FILENAME));}

AtcTableModel *AtcTableModel::_instance = 0;
namespace {
...
public:
    ...
    QString code() const {return m_Datas.value(AtcTableModel::ATC_Code);}
    QString english() const {return m_Datas.value(AtcTableModel::ATC_EnglishLabel);}
    QString french() const {return m_Datas.value(AtcTableModel::ATC_FrenchLabel);}
...
QStringList AtcTableModel::getAtcLabel(const QStringList &codes)
{
    QStringList list;
...
bool AtcTableModel::insertAtcCodeToDatabase(const QString &connectionName)
{
//    // Start transaction
