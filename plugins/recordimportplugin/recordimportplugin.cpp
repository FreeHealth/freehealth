...
using namespace RecordImport::Internal;

static inline Core::IUser *user()  { return Core::ICore::instance()->user(); }
static inline Core::ITheme *theme()  { return Core::ICore::instance()->theme(); }
static inline void messageSplash(const QString &s) {theme()->messageSplashScreen(s); }

RecordimportPlugin::RecordimportPlugin()
{
    setObjectName("RecordimportPlugin");
    if (Utils::Log::debugPluginsCreation())
        qWarning() << "creating recordimport";

    // Add Translator to the Application
    Core::ICore::instance()->translators()->addNewTranslator("recordimport");
...
bool RecordimportPlugin::initialize(const QStringList &arguments, QString *errorString)
{
    Q_UNUSED(arguments)
    ...
    //
    QAction *a = new QAction(tr("recordimport action"), this);
    a->setIcon(theme()->icon(Constants::RECORDIMPORT_ICON));
    Core::Command *cmd = am->registerAction(a, Constants::ACTION_ID,
                                            Core::Context(Core::Constants::C_GLOBAL));
    cmd->setTranslations(Constants::RECORD_IMPORT);
    ...
    connect(a, SIGNAL(triggered()), this, SLOT(openRecordImportDialog()));
...
void RecordimportPlugin::extensionsInitialized()
{
    if (Utils::Log::debugPluginsCreation()) {
        qWarning() << "recordimport::extensionsInitialized";
    }
    ...
    messageSplash(tr("Initializing recordimport..."));
...
void RecordimportPlugin::postCoreInitialization()
{
    if (Utils::Log::debugPluginsCreation())
        WARN_FUNC;
...
void RecordimportPlugin::openRecordImportDialog()
{
    qWarning() << Q_FUNC_INFO;
    RecordImport::RecordImportDialog dlg;
    dlg.exec();
    return;
}

QStringList RecordimportPlugin::softwareList()
{
    QStringList softwareList;
    softwareList << RecordImport::Constants::CIELCOMPTA // Index 0
                 << RecordImport::Constants::GESTCAB;   // Index 1
    return softwareList;
}

ExtensionSystem::IPlugin::ShutdownFlag RecordimportPlugin::aboutToShutdown()
{
    if (Utils::Log::debugPluginsCreation())
        WARN_FUNC;
    // Save settings
...
// void RecordimportPlugin::triggerAction()
// {
//     QMessageBox::information(Core::ICore::instance()->mainWindow(),
//                              tr("Action triggered"),
//                              tr("This is an action from recordimport."));
// }

void RecordimportPlugin::coreAboutToClose()
{
    if (Utils::Log::debugPluginsCreation())
        WARN_FUNC;
...
//void RecordimportPlugin::test_your_plugin_unit_test()
//{
...
Q_EXPORT_PLUGIN(recordimportPlugin)
