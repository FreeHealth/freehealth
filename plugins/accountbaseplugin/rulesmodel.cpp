...
{
public:
    RulesModelPrivate(RulesModel *parent) : m_SqlTable(0), m_IsDirty(false), q(parent)
    {
        m_SqlTable = new QSqlTableModel(q, QSqlDatabase::database(Constants::DB_ACCOUNTANCY));
        m_SqlTable->setTable(AccountDB::AccountBase::instance()->table(Constants::Table_Rules));
...
RulesModel::RulesModel(QObject *parent) :
        QAbstractTableModel(parent), d(new Internal::RulesModelPrivate(this))
{
...
void RulesModel::setUserUuid(const QString &uuid)
{
    QHash<int, QString> where;
    where.insert(Constants::BANKDETAILS_USER_UID, QString("='%1'").arg(uuid));
    d->m_SqlTable->setFilter(AccountBase::instance()->getWhereClause(Constants::Table_Rules, where));
}
...
bool RulesModel::insertRows(int row, int count, const QModelIndex &parent)
{
    d->m_IsDirty = true;
...
bool RulesModel::removeRows(int row, int count, const QModelIndex &parent)
{
    d->m_IsDirty = true;
...
void RulesModel::revert()
{
    d->m_IsDirty = false;
    d->m_SqlTable->revert();
}

bool RulesModel::isDirty() const
{
    return d->m_IsDirty;
}
