...
public:
    ...
    static LanguageList getTrLanguages()
    {
        LanguageList trLanguages;
        QStringList qms = findQmFiles(settings()->path(Core::ISettings::TranslationsPath));
...
public:
    LanguageModel(const LanguageList& languages, QObject* parent = 0)
            : QAbstractTableModel(parent), m_Languages(languages)
    {
    }
    ...
    QVariant data(const QModelIndex& index, int role = Qt::DisplayRole) const
    {
        ...
        if (role == Qt::DecorationRole) {
            QIcon icon = theme()->icon("flags/" + l.m_CountryCode + ".png", Core::ITheme::SmallIcon);
            return icon;
        }
...
public:
    ...
    void reset()
    {
        ...
        if (m_DisplayMode == LanguageComboBox::AllLanguages)
            m_Model = new LanguageModel(Language::getAllLanguages(), q);
        else
            m_Model = new LanguageModel(Language::getTrLanguages(), q);
