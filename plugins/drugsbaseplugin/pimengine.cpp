...
public:
    ...
    QString risk(const QString &lang = QString::null) const
    {
        ...
        QString r;
//        if (l=="fr")
//            r = base()->getLabel(m_Infos.value(PIM_RiskMasterLid).toInt(), l);
//        else
            r = base()->getLabel(m_Infos.value(PIM_RiskMasterLid).toInt(), "en");
...
bool PimEngine::init()
{
    ...
    req = base()->select(Constants::Table_PIM_SOURCES, Constants::PIM_SOURCES_SID);
    QSqlQuery source(DB);
    if (source.exec(req)) {
        while (source.next()) {
            PimSource pimSource;
...
static PimInteraction *getPimInteractionFromDb(const int sourceId, const int pimId, IDrugEngine *engine, bool getIcdRelatedPims = true)
{
    if (!getIcdRelatedPims) {
        ...
        int nbicd = base()->count(Constants::Table_PIMS_RELATED_ICD, Constants::PIMS_RELICD_PIM_ID, base()->getWhereClause(Constants::Table_PIMS_RELATED_ICD, where));
        if (nbicd > 0)
            return 0;
    }
    ...
    Utils::FieldList get;
    get << base()->fields(Constants::Table_PIMS);
    get << base()->fields(Constants::Table_PIM_SOURCES);
    get << base()->fields(Constants::Table_PIM_TYPES);
    ...
    req = base()->select(Constants::Table_PIMS_RELATED_ATC, joins, where);
    if (query.exec(req)) {
        while (query.next()) {
    ...
    req = base()->select(Constants::Table_PIMS_RELATED_ICD, joins, where);
    if (query.exec(req)) {
        while (query.next()) {
